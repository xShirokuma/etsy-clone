...
export const thunkAddFav = (productId, userId) => async (dispatch) => {
  const response = await fetch(
    `/api/users/${userId}/favorites/products/${productId}`,
    {
      method: "PUT",
...
export const thunkAddToCart =
  (sessionUser, product, value) => async (dispatch) => {
    const response = await fetch(
      `/api/users/${sessionUser.id}/cart/products/${product.id}/${value}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  switch (action.type) {
    ...
    case ADD_CART:
      ...
      return {
        ...
        user: {
          ...state.user,
          cart_session: { ...state.user.cart_session, cart: newCart },
        },
      }
    case UPDATE_CART:
      let updatedCart = state.user.cart_session.cart
      index = state.user.cart_session.cart.findIndex(
        (ele) => ele.productId === action.updatedCartItem.productId
      )
      ...
      return {
        ...
        user: {
          ...state.user,
          cart_session: { ...state.user.cart_session, cart: updatedCart },
        },
      }
